...
void setup() {
  ...
  mcp.begin(0);
  for(int i = 0; i <= 16; i++) { mcp.pinMode(i, OUTPUT); mcp.digitalWrite(i, LOW);}
}
...
void loop() {
  ...
  for(int i = 0; i < 20; i++){
    if(i <=3) { //arduino internal pins
      digitalWrite(i+2, cmri.get_bit(i));
    }
    else {
      mcp.digitalWrite(i-4, cmri.get_bit(i));
    }
  }
